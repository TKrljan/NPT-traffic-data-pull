...
for id_ in specific_ids:
    sanitized_id = id_.replace(':', '_')
    csv_path = os.path.join(output_folder, f"{sanitized_id}_data.csv")
    if os.path.exists(csv_path):
        dataframes[id_] = pd.read_csv(csv_path)
    else:
        dataframes[id_] = pd.DataFrame(columns=['measurement_time', 'TrafficHeadway', 'TrafficFlow', 'TrafficSpeed', 'TrafficConcentration'])
